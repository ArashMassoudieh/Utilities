...
{
    if (BTC_p.size() != BTC_d.size() || BTC_p.size()==0) {
    ...
    for (size_t i = 0; i < n; ++i) {
        covariance += (BTC_p.getValue(i) - meanX) * (BTC_d.getValue(i) - meanY);
    }

    return covariance / double(n);
}

#include <cstddef>

template <typename T>
T covariance_mapped(const TimeSeries<T>& ts1,
                    const TimeSeries<T>& ts2,
                    int start_item,
                    bool sample)
{
    // Ensure we have points to work with
    if (start_item < 0) start_item = 0;
    if (static_cast<size_t>(start_item) >= ts1.size()) return T{};

    // Two-pass is fine, but use a single-pass (Welford) for better numerical stability.
    // Bivariate Welford algorithm:
    // https://en.wikipedia.org/wiki/Algorithms_for_calculating_variance#Covariance
    size_t n = 0;
    T mean_x = T{};
    T mean_y = T{};
    T C = T{}; // running sum for covariance accumulator (not yet divided)

    for (size_t i = static_cast<size_t>(start_item); i < ts1.size(); ++i) {
        const T x = ts1[i].c;
        const T t = ts1[i].t;
        const T y = ts2.interpol(t);

        ++n;
        const T dx = x - mean_x;
        mean_x += dx / static_cast<T>(n);
        // mean_y uses the *old* mean_x only for dx; but for bivariate Welford we update mean_y similarly:
        const T dy = y - mean_y;
        mean_y += dy / static_cast<T>(n);

        // Update C after updating means using the old deviations
        C += dx * (y - mean_y); // equivalent stable form
    }

    if (n == 0) return T{};
    if (sample) {
        return (n > 1) ? (C / static_cast<T>(n - 1)) : T{};
    } else {
...
template<class T>
TimeSeries<T> TimeSeries<T>::inverse_cummulative_uniform(int nintervals) const
{
    TimeSeries<T> out = inverse();
...
template<typename T>
TimeSeries<T> operator/(const TimeSeries<T>& ts, T alpha) {
    TimeSeries<T> result = ts;        // copies structured_, dt_, etc.
    for (auto& p : result) p.c /= alpha;
    return result;
}
...
template<typename T>
TimeSeries<T> TimeSeries<T>::GetCummulativeDistribution(bool reverse) {
    return static_cast<const TimeSeries<T>&>(*this).GetCummulativeDistribution(reverse);
}
...
template<typename T>
std::vector<T> TimeSeries<T>::tToStdVector() const
{
    std::vector<T> out;
    for (size_t i = 0; i < this->size(); ++i)
    {
        out.push_back(getTime(i));
    }
    return out;
}

template<typename T>
std::vector<double> TimeSeries<T>::ValuesToStdVector() const
{
    std::vector<T> out;
    for (size_t i = 0; i < this->size(); ++i)
    {
...
template<typename T>
T TimeSeries<T>::fitExponentialDecay() const
{
    if (this->size() < 2) {
    ...
    if (valid_points < 1) {
        throw std::runtime_error("fitExponentialDecay: no valid positive values");
    }

    if (std::abs(sum_neg_ln_y) < 1e-15) {
        throw std::runtime_error("fitExponentialDecay: all values are 1 (no decay)");
    }

    T l = sum_x / sum_neg_ln_y;

    if (l <= 0.0) {
        throw std::runtime_error("fitExponentialDecay: negative decay length");
    }
