...
//			return;
		...
		if (s.size()>=2)
		if ((s[0].substr(0,2)!="//") && (aquiutils::tolower(s[0])!="names") && s[0].substr(0,1)!="n" && s[0].substr(0,1)!="t")
		{
...
CTimeSeries CTimeSeries::make_uniform(unsigned int numincrements)
{
	structured = false;
	CTimeSeries out;
	double dt = (t[n-1]-t[0])/double(numincrements);
	for (int i=0; i<numincrements+1; i++)
	{
        out.append(t[0]+i*dt,interpol(t[0]+i*dt));
	}
